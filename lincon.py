...
def lincon(formula, data, tr=0.2, alpha=0.05, method="hochberg", **kwargs):
    con = 0
    pr = True
    crit = np.nan
    KB = False
    ...
    x = np.split(mf.response, mf.iloc[:, 1])
                ...
                if j < k:
                    ...
                    if not KB:
                        ...
                        if not flag or CC > 28:
                            crit = smmvalv2(dfvec=np.repeat(df, CC), alpha=alpha)
    ...
    if np.sum(con**2) > 0:
        ...
        for d in range(con.shape[1]):
            ...
            if not flag:
                crit = smmvalv2(dfvec=np.repeat(df, con.shape[1]), alpha=alpha)
    ...
    psihat[:, 5] = pairwise_tukeyhsd(psihat[:, 5], method=method)
    fnames = mf.iloc[:, 1].unique()
    result = {"comp": psihat, "fnames": fnames, "alpha": alpha, "call": cl}
    return result